...
# ===========================================================================
    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        def bereken_periode(p_start, p_end, is_maandweergave):
            omzet = float(sum(r["amount"] for r in income_rows if p_start <= r["date"] <= p_end))
            kosten = 0.0
            for c in cost_rows:
                if not c["due_date"] or c["due_date"] > p_end:
                    continue
                if c["frequency"] == "Eenmalig":
                    if p_start <= c["due_date"] <= p_end:
                        kosten += float(c["amount"])
                elif c["frequency"] == "Maandelijks":
                    kosten += float(c["amount"]) if is_maandweergave else float(c["amount"]) * 12 / 52
                elif c["frequency"] == "Jaarlijks":
                    if is_maandweergave and p_start.month == c["due_date"].month:
                        kosten += float(c["amount"])
            return omzet, kosten
        ...
        label_suffix = "incl. schatting" if include_schatting else "alleen gerealiseerd"
