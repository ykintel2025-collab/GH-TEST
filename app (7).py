...
if page == "🏠 Dashboard":
    ...
    if recent_docs:
        st.warning(f"📎 {len(recent_docs)} document(en) recent geüpload (laatste 3 dagen) — bekijk hieronder.")
        with st.expander(f"🗂️ Recent geüpload ({len(recent_docs)})", expanded=True):
            ...
            for doc in recent_docs:
                dcol1, dcol2, dcol3, dcol4 = st.columns([2.2, 1.6, 1.2, 0.8])
    ...
    if not contacts:
        ...
        for c in contacts:
            ...
            with st.expander(label):
                ...
                docs = db.get_documents(contact_id=c["id"])
                if docs:
                    for d in docs:
                        dcol1, dcol2, dcol3 = st.columns([3, 1, 0.5])
