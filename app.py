...
# ===========================================================================
        ...
        with col_pipeline:
            ...
            with st.popover("➕ Deal, financiering of verwachte opdracht toevoegen"):
                with st.form("add_pipeline_form", clear_on_submit=True):
                    ...
                    potential_value = pcol1.number_input("Verwachte omzet / leenbedrag", min_value=0.0, step=500.0)
                    expected_date = pcol2.date_input("Verwachte datum (voor prognose)", value=None)
                    ...
                    if st.form_submit_button("Toevoegen") and company:
                        db.add_pipeline(
                            ...
                            deal_type=deal_type, expected_date=expected_date,
                        )
            ...
            if pipeline_rows:
                for item in pipeline_rows:
                    ...
                    with st.expander(f"{type_icon} {item['company']} — {item['status']} ({eur(item['potential_value'])}) — verwacht: {verwacht}"):
                        ...
                        new_expected = st.date_input(
                            "Verwachte datum (voor prognose)", value=item.get("expected_date"), key=f"pexp_{item['id']}",
                        )
                        st.caption(f"Volgende actie: {item['next_action'] or '—'}")
                        pc1, pc2 = st.columns(2)
                        if pc1.button("Opslaan", key=f"save_pstatus_{item['id']}"):
                            db.update_pipeline(item["id"], status=new_p_status, expected_date=new_expected)
                            st.rerun()
    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        def pipeline_bedrag_in_periode(item, p_start, p_end, is_maandweergave):
            it = {"date": item.get("expected_date"), "amount": item.get("potential_value") or 0, "frequency": "Eenmalig"}
