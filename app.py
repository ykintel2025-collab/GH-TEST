...
# ===========================================================================
        ...
        with col_income:
            ...
            if income_rows:
                for i in income_rows:
                    ...
                    icol1.write(f"{test_marker}{i['date'].strftime('%d-%m-%Y')} — {i['source']}{freq_marker}")
    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        include_schatting = col_toggle1.checkbox(
            "📊 Inclusief schatting (contracten uit Pipeline & Inkomsten)",
            value=False,
            help="Standaard toont dit scherm alleen wat je écht hebt geregistreerd. "
                 "Zet dit aan om ook te zien hoe het beeld eruitziet mét de geschatte contractopbrengsten erbij.",
        )
        include_fictief = col_toggle2.checkbox(
            "🧪 Inclusief fictieve / test-inkomsten",
            value=False,
            help="Inkomsten die je als 'fictief/test' hebt gemarkeerd tellen standaard niet mee. "
                 "Zet dit aan om ermee te spelen zonder dat het je echte cijfers verstoort.",
        )

        income_rows_filtered = [r for r in income_rows if include_fictief or not r.get("is_test")]

        streams = db.get_revenue_streams()
        verwachte_omzet_maand_totaal = float(sum(s["budgeted_amount"] for s in streams)) / 12

        if include_schatting and streams:
            with st.expander("🔍 Waar komt de schatting vandaan? (per maand)"):
                for s in streams:
                    maand_bedrag = float(s["budgeted_amount"]) / 12
                    st.write(f"**{s['name']}** — {eur(maand_bedrag)}/maand  ·  _{s.get('notes') or ''}_")
                st.caption(f"Totaal: {eur(verwachte_omzet_maand_totaal)}/maand")

        def bereken_herhaling(items, p_start, p_end, is_maandweergave, date_key):
            """Telt een lijst met bedragen (inkomsten of kosten) op voor een periode,
            rekening houdend met eenmalig/wekelijks/maandelijks/jaarlijks."""
            totaal = 0.0
            for it in items:
                d = it.get(date_key)
                if not d or d > p_end:
        ...
        def bereken_periode(p_start, p_end, is_maandweergave):
            omzet = bereken_herhaling(income_rows_filtered, p_start, p_end, is_maandweergave, "date")
            kosten = bereken_herhaling(cost_rows, p_start, p_end, is_maandweergave, "due_date")
            return omzet, kosten

        rows = []
        for p in months:
            omzet_gerealiseerd, kosten = bereken_periode(p["start"], p["end"], True)
            verwachte_omzet = verwachte_omzet_maand_totaal if include_schatting else 0.0
            privé = privé_total_monthly
            resultaat = omzet_gerealiseerd + verwachte_omzet - kosten
            aflossing = resultaat - privé
            rows.append({
                "Periode": p["label"], "start": p["start"], "end": p["end"],
                "Omzet (gerealiseerd)": omzet_gerealiseerd,
                "Verwachte omzet (schatting)": verwachte_omzet,
                "Bedrijfskosten": -kosten, "Resultaat": resultaat,
                "Privé-uitgaven": -privé, "Aflossingscapaciteit": aflossing,
            })

        if include_schatting:
            line_items = [
                "Omzet (gerealiseerd)", "Verwachte omzet (schatting)", "Bedrijfskosten",
                "Resultaat", "Privé-uitgaven", "Aflossingscapaciteit",
            ]
        else:
            line_items = ["Omzet (gerealiseerd)", "Bedrijfskosten", "Resultaat", "Privé-uitgaven", "Aflossingscapaciteit"]
        table_data = {"Onderdeel": line_items}
        for r in rows:
            table_data[r["Periode"]] = [eur(r[item]) for item in line_items]
        ...
        suffix_parts = []
        suffix_parts.append("incl. schatting" if include_schatting else "excl. schatting")
        if include_fictief:
            suffix_parts.append("incl. fictief")
        label_suffix = ", ".join(suffix_parts)
        st.metric(f"Totale aflossingscapaciteit ({n_months} maanden, {label_suffix})", eur(totaal_aflossing))
        st.caption(
            "Omzet (gerealiseerd) = daadwerkelijk geregistreerde inkomsten in die maand. "
            "Verwachte omzet (schatting) = de contracten bij Financieel → Pipeline & Inkomsten, "
            "gelijkmatig verdeeld over het jaar. Bedrijfskosten = lopende kosten (incl. salaris), "
        ...
        week_rows = []
        for w in weken:
            # alleen het deel van de week dat ook echt in de gekozen maand valt
            w_start_clip = max(w["start"], gekozen["start"])
            w_end_clip = min(w["end"], gekozen["end"])
            omzet, kosten = bereken_periode(w_start_clip, w_end_clip, False)
            verwachte_omzet_week = (verwachte_omzet_maand_totaal * 12 / 52) if include_schatting else 0.0
            ...
            week_rows.append({
                "Week": w["label"], "Omzet": omzet, "Schatting": verwachte_omzet_week,
        ...
        st.dataframe(
            ...
            column_config={c: st.column_config.NumberColumn(format="€ %.0f") for c in
                            ["Omzet", "Schatting", "Kosten", "Privé", "Aflossing"]},
        )
        st.caption(
