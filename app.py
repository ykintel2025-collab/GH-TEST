...
# ===========================================================================
    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        verwachte_omzet_jaar = float(sum(s["budgeted_amount"] for s in db.get_revenue_streams()))
        verwachte_omzet_per_period_vol = verwachte_omzet_jaar / 12 if view_mode == "Per maand" else verwachte_omzet_jaar / 52
        verwachte_omzet_per_period = verwachte_omzet_per_period_vol if include_schatting else 0.0

        rows = []
        for p in periods:
            omzet_gerealiseerd = float(sum(r["amount"] for r in income_rows if p["start"] <= r["date"] <= p["end"]))
            ...
            resultaat = omzet_gerealiseerd + verwachte_omzet_per_period - kosten
            aflossing = resultaat - privé_per_period
            rows.append({
                "Periode": p["label"],
                "Omzet (gerealiseerd)": omzet_gerealiseerd,
                "Verwachte omzet (schatting)": verwachte_omzet_per_period,
                "Bedrijfskosten": -kosten,
                "Resultaat": resultaat,
                "Privé-uitgaven": -privé_per_period,
        ...
        periode_naam = "weken" if view_mode == "Per week" else "maanden"
        label_suffix = "incl. schatting" if include_schatting else "alleen gerealiseerd"
        st.metric(f"Totale aflossingscapaciteit ({n_periods} {periode_naam}, {label_suffix})", eur(totaal_aflossing))
        st.caption(
            "Omzet (gerealiseerd) = daadwerkelijk geregistreerde inkomsten in die periode. "
            "Verwachte omzet (schatting) = de contracten bij Financieel → Pipeline & Inkomsten, "
            "gelijkmatig verdeeld over het jaar — dit is nadrukkelijk een schatting, geen realisatie. "
            "Bedrijfskosten = lopende kosten (incl. salaris), waarbij 'Maandelijks' automatisch elke "
            "periode terugkomt vanaf de ingevulde vervaldatum. Privé-uitgaven worden gelijkmatig verdeeld "
            "over de periodes (bij weekweergave: maandbedrag × 12 ÷ 52)."
        )
