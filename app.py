...
if page == "🏠 Dashboard":
    ...
    st.caption("Taken, lopende kosten en betaaldatums van schulden — per week of maand.")

    agenda_items = []
    for t in db.get_tasks(status="Open"):
        if t["due_date"]:
            agenda_items.append({"date": t["due_date"], "label": f"✅ {t['title']}", "detail": t["assigned_to"]})
    for c in db.get_running_costs(status="Open"):
        if c["due_date"]:
            agenda_items.append({"date": c["due_date"], "label": f"🧾 {c['name']}", "detail": eur(c["amount"])})
    for d in db.get_debts(status="Open"):
        if d.get("next_payment_date"):
            agenda_items.append({
                "date": d["next_payment_date"], "label": f"📌 Betaling {d['creditor_name']}",
                "detail": d.get("payment_agreement") or eur(d["current_amount"]),
            })

    if not agenda_items:
        st.info("Geen geplande taken, kosten of betaaldatums gevonden.")
    else:
...
# ===========================================================================
    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        st.caption(
            "Inkomsten min lopende kosten (incl. salaris) = aflossingscapaciteit per periode. "
            "Schulden staan hier bewust buiten — de verdeling over schuldeisers bepaal je zelf, "
            "via 'Betaling registreren' bij de betreffende schuldeiser."
        )

        cashflow = db.get_cashflow_periods()
        income_rows = cashflow["income"]
        cost_rows = cashflow["costs"]

        if not income_rows and not cost_rows:
            st.info("Nog geen inkomsten of lopende kosten met datum gevonden.")
        else:
            ...
            def period_key(d):
                if view_mode == "Per week":
                    ...
                    return f"{year}-W{week:02d}", f"Week {week} — {year}"
                else:
                    return d.strftime("%Y-%m"), d.strftime("%B %Y")

            periods = {}
            for r in income_rows:
                key, label = period_key(r["date"])
                periods.setdefault(key, {"label": label, "income": 0.0, "costs": 0.0})
                periods[key]["income"] += r["amount"]
            for r in cost_rows:
                key, label = period_key(r["date"])
                periods.setdefault(key, {"label": label, "income": 0.0, "costs": 0.0})
                periods[key]["costs"] += r["amount"]

            rows = []
            for key in sorted(periods.keys()):
                p = periods[key]
                capaciteit = p["income"] - p["costs"]
                rows.append({
                    "Periode": p["label"], "Inkomsten": p["income"], "Lopende kosten": p["costs"],
                    "Aflossingscapaciteit": capaciteit,
                })
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Lopende kosten": st.column_config.NumberColumn(format="€ %.0f"),
                    "Aflossingscapaciteit": st.column_config.NumberColumn(format="€ %.0f"),
                },
            )

            totaal_capaciteit = cdf["Aflossingscapaciteit"].sum()
            st.metric("Totale aflossingscapaciteit (alle getoonde periodes)", eur(totaal_capaciteit))
            st.caption(
                "Let op: lopende kosten tellen mee op hun ingevulde vervaldatum. Voor 'Maandelijks' terugkerende "
                "kosten moet je dus zelf steeds de eerstvolgende vervaldatum bijwerken — een automatische "
                "herhaling kan ik alsnog bouwen als je dat handig vindt."
            )
