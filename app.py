    ...
    # --- Onderdeel: Liquiditeit ------------------------------------------------
        ...
        show_prognose = col_toggle1.checkbox(
            ...
            help="Standaard toont dit scherm alleen wat je écht hebt geregistreerd — de harde waarheid. "
                 "Zet dit aan om ook de geschatte contractopbrengsten en eventuele fictieve/test-inkomsten "
                 "mee te nemen, voor een optimistischer 'wat-als'-beeld.",
        )
        if show_prognose:
            col_toggle2.caption("Prognose = 📊 geschatte contracten + 🧪 fictieve/test-inkomsten, beide meegeteld.")
        else:
        ...
        if show_prognose and streams:
            with st.expander("🔍 Prognose per opbrengstenbron (deze maand)"):
                for s in streams:
                    bedrag = stream_bedrag_in_periode(s, months[0]["start"], months[0]["end"], True)
                    st.write(f"**{s['name']}** — {eur(bedrag)}  ·  _{s.get('notes') or ''}_")
        ...
        for p in months:
            ...
            row.update({f"↳ {naam} (prognose)": bedrag for naam, bedrag in stream_bedragen.items()})
            row.update({
        ...
        line_items = ["Omzet (gerealiseerd)"]
        if show_prognose:
            line_items += [f"↳ {s['name']} (prognose)" for s in streams]
        ...
        st.caption(
            "Omzet (gerealiseerd) = daadwerkelijk geregistreerde inkomsten. Prognose-regels (↳) = per "
            "opbrengstenbron, op basis van de planning die je bij Pipeline & Inkomsten hebt ingesteld. "
            "Bedrijfskosten = lopende kosten (incl. salaris), 'Maandelijks' komt elke maand automatisch "
            "terug. Privé-uitgaven = je maandbudget."
        )
