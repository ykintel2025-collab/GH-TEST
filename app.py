...
if page == "🏠 Dashboard":
    ...
    if tasks_no_date:
        ...
        for t in tasks_no_date:
            ...
            if cols[1].button(label, key=f"opentask_nodate_{t['id']}", use_container_width=True):
                show_task_dialog(t)
            if done:
        ...
        for key, items in groups.items():
            ...
            for item in items:
                ...
                if item["kind"] == "task":
                    ...
                    task_label = f"{prefix}`{item['date'].strftime('%d-%m')}` {item['label']} — {item['detail']}"
                    if tcol2.button(task_label, key=f"opentask_agenda_{item['id']}", use_container_width=True):
                        show_task_dialog(item["task"])
                    if done:
                        db.update_task(item["id"], status="Done")
                        st.rerun()
                elif item["kind"] == "cost":
                    cost_label = f"{prefix}`{item['date'].strftime('%d-%m')}` {item['label']} — {item['detail']}"
                    if st.button(cost_label, key=f"opencost_{item['id']}", use_container_width=True):
                        st.session_state["nav_page"] = "💰 Financieel"
                        st.session_state["financieel_subpage"] = "🧾 Lopende kosten"
                        st.rerun()
                else:
                    debt_label = f"{prefix}`{item['date'].strftime('%d-%m')}` {item['label']} — {item['detail']}"
                    if st.button(debt_label, key=f"opendebt_{item['id']}", use_container_width=True):
                        st.session_state["nav_page"] = "💰 Financieel"
                        st.session_state["financieel_subpage"] = "📋 Schulden"
...
# ===========================================================================
    ...
    if not tasks:
        ...
        for t in tasks:
            ...
            if cols[1].button(title_txt, key=f"opentask_main_{t['id']}", use_container_width=True):
                show_task_dialog(t)
            if t["description"]:
