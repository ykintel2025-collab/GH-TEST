...
def add_pipeline(company, status, potential_value, next_action, owner, deal_type="Business", expected_date=None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO pipeline (company, status, potential_value, next_action, owner, deal_type, expected_date)
           VALUES (%s, %s, %s, %s, %s, %s, %s)""",
        (company, status, potential_value, next_action, owner, deal_type, expected_date),
    )
    conn.commit()
    cur.close()
    conn.close()
...
def update_pipeline(pipeline_id, **fields):
    if not fields:
        return
    allowed = {"company", "status", "potential_value", "next_action", "owner", "deal_type", "expected_date"}
    keys = [k for k in fields if k in allowed]
    if not keys:
        return
    set_clause = ", ".join(f"{k} = %s" for k in keys)
...
def get_cashflow_periods():
    """Inkomsten en lopende kosten met datum, voor de liquiditeitsbegroting (week/maand)."""
    conn = get_connection()
    cur = _dict_cursor(conn)
    cur.execute("SELECT date, amount FROM income")
    income_rows = [dict(r) for r in cur.fetchall()]
    cur.execute("SELECT due_date AS date, amount FROM running_costs WHERE due_date IS NOT NULL")
    cost_rows = [dict(r) for r in cur.fetchall()]
    cur.close()
    conn.close()
    ...
    for r in cost_rows:
        r["amount"] = float(r["amount"])
    return {"income": income_rows, "costs": cost_rows}
