...
def add_task(title, assigned_to, created_by, description=None, due_date=None, related_debt_id=None, related_contact_id=None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO tasks (title, description, assigned_to, due_date, related_debt_id, related_contact_id, created_by)
           VALUES (%s, %s, %s, %s, %s, %s, %s)""",
        (title, description, assigned_to, due_date, related_debt_id, related_contact_id, created_by),
    )
    conn.commit()
    cur.close()
    release_connection(conn)
...
def update_task(task_id, **fields):
    if not fields:
        return
    allowed = {"title", "description", "assigned_to", "due_date", "related_debt_id", "related_contact_id", "status"}
    keys = [k for k in fields if k in allowed]
    if not keys:
        return
    set_clause = ", ".join(f"{k} = %s" for k in keys)
...
def get_tasks(assigned_to=None, status=None):
    ...
    cur = _dict_cursor(conn)
    query = """
        SELECT t.*, d.creditor_name, c.name AS contact_name
        FROM tasks t
