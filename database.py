...
def _get_pool():
    ...
    return psycopg2.pool.ThreadedConnectionPool(1, 10, st.secrets["DB_URL"])


def get_connection():
    """Haalt een verbinding uit de pool. Gebruik altijd samen met release_connection(conn)
    (nooit conn.close() rechtstreeks, dat zou de pool-verbinding definitief afsluiten)."""
    return _get_pool().getconn()


def release_connection(conn):
    """Geeft een verbinding terug aan de pool i.p.v. 'm af te sluiten."""
...
def update_debt(debt_id, **fields):
    if not fields:
        return
    allowed = {"contact_id", "total_amount", "current_amount", "priority", "status", "last_contact", "payment_agreement", "next_payment_date"}
...
def get_debts(status=None):
    ...
    base_query = """
        SELECT d.id, d.contact_id, d.total_amount, d.current_amount, d.priority, d.status, d.last_contact,
               d.payment_agreement, d.next_payment_date,
...
def add_document(filename, storage_path, uploaded_by, contact_id=None, file_size=None, notes=None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO documents (filename, storage_path, contact_id, uploaded_by, file_size, notes)
           VALUES (%s, %s, %s, %s, %s, %s)""",
        (filename, storage_path, contact_id, uploaded_by, file_size, notes),
    )
    conn.commit()
    cur.close()
    release_connection(conn)
...
def get_documents(contact_id=None):
    ...
    gebruik contact_id='geen' om alleen de 'algemene' documenten (niet aan een contact gekoppeld) te krijgen."""
    conn = get_connection()
    cur = _dict_cursor(conn)
    if contact_id == "geen":
        cur.execute("SELECT * FROM documents WHERE contact_id IS NULL ORDER BY uploaded_at DESC")
    elif contact_id is not None:
        cur.execute("SELECT * FROM documents WHERE contact_id = %s ORDER BY uploaded_at DESC", (contact_id,))
    else:
